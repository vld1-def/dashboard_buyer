...
-- ───────────────────────────────────────────────────────────
--  ЗВ'ЯЗОК ІЗ ТВОЄЮ НАЗВОЮ КАБІНЕТА
-- ───────────────────────────────────────────────────────────
--  У accounts_mapping кабінети підписані так, як їх звеш ти. У
--  Facebook вони під номерами. Сторінка намагається звести їх
--  сама — по збігу номера або назви. Ця колонка потрібна на
--  той випадок, коли не звелось: тоді в панелі кабінета
--  з'являється список «Facebook cabinet», і вибір запам'ятовується сюди.
--
--  Без неї все працює, просто ручної прив'язки не буде.
--  Виконайте одразу — це один рядок, і він нічого не ломає.
-- ───────────────────────────────────────────────────────────
