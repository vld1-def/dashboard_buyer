...
--
--  ⚠️ ПОСИЛАННЯ ДЛЯ ТІМЛІДА ПЕРЕСТАНЕ ПРАЦЮВАТИ
--  У дашборді є обхід: якщо в адресі ?view=teamlead, логін пропускається,
--  бо читання було публічним. Після цих політик публічного читання не
--  лишається — тімлід побачить порожній екран і не матиме де ввійти.
--  Коли тімлід знадобиться, йому треба буде завести власного користувача
--  й роль, а не окреме посилання.
--
...
-- Домени: унікальність рахуємо по ВЛАСНИКУ, а не по команді. Інакше, якщо
-- два баєри сидять під однією назвою команди, другий не зможе додати
-- домен, який уже є в першого, — і не зрозуміє чому, бо чужий рядок йому
-- не видно.
drop index if exists domains_team_domain_idx;
create unique index if not exists domains_owner_domain_idx
  on public.domains (created_by, lower(domain));
